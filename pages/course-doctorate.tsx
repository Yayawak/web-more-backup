...
import { doctorateDepartment } from '@/constants/doctorateDepartment'
import { ICurriculum, IDepartmentCard, IProgram } from '@/types/department'
import { NextPage } from 'next'
import Image from 'next/image'
import Link from 'next/link'

const DepartmentCard = ({ logoPath, name, describe, curriculum }: IDepartmentCard) => {
  return (
    <div className={`p-4 lg:px-3 bg-white rounded-xl`}>
      <div className={`flex flex-col items-center mb-4 md:h-44 lg:h-48 xl:h-44`}>
        <div>
          <Image
            src={logoPath}
            alt='department logo'
      ...
      <Container className="my-4 md:my-8 lg:mt-16 lg:mb-48">
        ...
        <div className='flex flex-col items-center'>
          <div className='h-fit mx-4 grid grid-cols-1 gap-4 lg:gap-5 md:grid-cols-3 md:auto-rows-fr lg:max-w-[90%]'>
            {doctorateDepartment.map((department) => {
