...
import { MessageModel } from '../model/news_model'

const Home: NextPage = () => {
  const { data, isLoading, isSuccess } = useGetHomeNewsQuery(8)

  if (isSuccess) {
    const message: MessageModel[] = data.message
    return (
      <>
        ...
        <Container className="news-cards grid justify-between gap-[16px] mt-[32px] grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 flex-wrap">
          {message.map((d, i) => (
            <NewsCard
              key={i}
              imageUrl={`https://www.science.kmitl.ac.th${d.news_file_topic
                .replace('public', ' ')
                .trim()}`}
              category={['วิชาการ', 'อบรม', 'สัมมนา']}
              title={d.topicFull}
              date={d.datetimepost_Preview}
              read={`https://www.science.kmitl.ac.th/page#/news_each/${d._id}`}
            />
          ))}
        </Container>
                ...
                <div>
                  ...
                  <ul>
                    <li className="ml-[24px] mt-[16px] list-disc">
                      รถโดยสารประจำทางสาย 1013 ต้นทาง จากซอยสุขุมวิท 77
                      (อ่อนนุช) ปลายทาง ตลาดหัวตะเข้
                    </li>
